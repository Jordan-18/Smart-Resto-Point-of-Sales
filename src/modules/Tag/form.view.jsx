...
import { getTag, getTagbyId, createTag, updateTag } from './index.service.js'
...
function Tag(){
    ...
    async function setBreadcrumb(){
        ...
        if(id != undefined){
            ...
            const result = await getTagbyId({api: globalData.api, token: globalData.token, id: id})
            setfoodData({
    ...
    async function onSubmit(event){
        ...
        if(BtnSubmit == 'Create'){
            const status = createTag({event, api: globalData.api, token: globalData.token, description: description, setLoadingSubmit})
            if(status){
                navigate('/master/food')
            }
        }else if(BtnSubmit == 'Update'){
            const status = updateTag({event, api: globalData.api, token: globalData.token, id: id, description: description, setLoadingSubmit})
            if(status){
                                    ...
                                    <form className="form w-100" id="dish_form" onSubmit={onSubmit}>
                                        <div className='row'>
                                            ...
                                            <div className="col-md-12 mb-3">
                                                ...
                                                <input type="text" className="form-control" id="dish_galleries" name="dish_galleries" defaultValue={foodData.dish_galleries}/>
                                            </div>
                                        </div>
                                        <div className='row'>
