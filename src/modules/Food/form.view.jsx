...
function Food(){
    ...
    async function onSubmit(event){
        if(BtnSubmit == 'Create'){
            const status = createUser({event, api: globalData.api, token: globalData.token})
            if(status){
                navigate('/setting/user')
            }
        }else if(BtnSubmit == 'Update'){
            const status = updateeUser({event, api: globalData.api, token: globalData.token, id: id})
            if(status){
                                    ...
                                    <form className="form w-100" id="user_form" onSubmit={onSubmit}>
                                        ...
                                        <div className='row'>
                                            <div className="col-md-12 mb-3" style={{textAlign: 'right'}}>
                                                <button type="submit" className={`btn btn-${BtnSubmit === 'Create' ? 'primary' : 'success'}`}>{BtnSubmit}</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
